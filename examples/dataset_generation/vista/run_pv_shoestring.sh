...
# 100,000 pv_shoestring volumes of 128 x 128 x 64 from ../config_full_pv_shoestring_v3.json (v3: uncropped, depth 3-16 m,
...
# Measured 2026-09-22 on the 144-volume preview with the node fully loaded: 14 s per volume on one
# core -> 389 core-hours = 2.7 node-hours at 144 ranks per node; on 1 node(s) about 2.7 h wall,
# walltime 04:00 gives a 1.3x margin. Each rank takes every N-th job of the shuffled Sobol job list.
...
srun --cpu-bind=cores env -u LD_PRELOAD "$PY" -m resmill.dataset.cli examples/dataset_generation/config_full_pv_shoestring_v3.json
