...
# 100,000 sh_proximal volumes of 128 x 128 x 64 from ../config_full_sh_proximal.json (128 x 128 x 64 stored whole, depth 3-16 m,
...
# Measured 2026-09-22 on the 144-volume preview with the node fully loaded: 18 s per volume on one
# core -> 503 core-hours = 3.5 node-hours at 144 ranks per node; on 1 node(s) about 3.5 h wall,
...
srun --cpu-bind=cores env -u LD_PRELOAD "$PY" -m resmill.dataset.cli examples/dataset_generation/config_full_sh_proximal.json
