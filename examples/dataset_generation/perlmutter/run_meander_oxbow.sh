...
# 100,000 meander_oxbow volumes of 128 x 128 x 64 from ../config_full_meander_oxbow_v3.json (v3), ResMill 5bf93ad or later,
# on NERSC Perlmutter CPU nodes: 128 single-thread ranks per node as in v1.
# Cost from the Vista measurement of 2026-09-22 (427 s a volume on one Grace core,
# assumed 1.3x slower per core here): 15,412 core-hours = 120.4 node-hours,
# about 3.9 h on 31 node(s); walltime 05:30 leaves a 1.3x margin.
...
srun --cpu-bind=cores python -m resmill.dataset.cli examples/dataset_generation/config_full_meander_oxbow_v3.json
