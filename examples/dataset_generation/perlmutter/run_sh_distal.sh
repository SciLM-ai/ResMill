...
# v2 dataset, 100,000 sh_distal samples from ../config_full_sh_distal_v2.json (ResMill 3e71977 or later),
# on NERSC Perlmutter CPU nodes: 128 single-thread ranks per node as in v1.
# Cost from the Vista measurement of 2026-09-22 (18.8 s a cube on one Grace core,
# assumed 1.3x slower per core here): 679 core-hours = 5.3 node-hours,
# about 1.8 h on 3 node(s); walltime 03:00 leaves a 1.3x margin.
...
srun --cpu-bind=cores python -m resmill.dataset.cli examples/dataset_generation/config_full_sh_distal_v2.json
