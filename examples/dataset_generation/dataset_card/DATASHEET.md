# Datasheet for *Siliciclastic Reservoirs*
...
Created by SciLM as part of research on AI-driven subsurface modeling.

**Who funded the creation of the dataset?**
TACC computing time (Vista). No direct industry funding; no third-party data dependencies.
...
**Fully self-contained.** No external links, no external assets. The engine code (https://github.com/ — to be released alongside this dataset) is independent open-source software; users do not need to run it to use this dataset.
...
Yes — the entire pipeline (engine, sampler, dataset writer, splitter) is in the open-source repository.
...
At time of release: not yet for downstream tasks. The dataset has been validated through the `plot_dataset.py` and `plot_dataset_stats.py` scripts (visual + statistical QA across 100 samples per architecture).
...
HuggingFace Datasets (`SciLM/SiliciclasticReservoirs`). Total size ~735 GB across 8 layer-type subdirectories.
...
SciLM, with hosting on HuggingFace.

**How can the owner/curator/manager of the dataset be contacted?**
Through the HuggingFace dataset issue tracker, or through the GitHub repository.
...
Yes — the engine code is open-source. Anyone can:
