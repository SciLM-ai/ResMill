...
- `ChannelLayer(n_sources=N)`: N entry points on the upstream edge, drawn
  ...
  channel enters at one of them; a preset that builds one channel per level
  (no avulsion, MEANDER_OXBOW) builds one belt per source per level, each
  with the level's full event budget. Default 1 leaves the published
  channels bit-identical. `examples/dataset_generation/config_full_<preset>_v2.json`
